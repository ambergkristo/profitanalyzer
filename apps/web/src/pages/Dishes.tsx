...
export function DishesPage() {
  ...
  const loadDishes = useCallback(async () => {
    const [dishes, actions, overview] = await Promise.all([
    ...
    <div className="space-y-6">
      <section className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <div className="rounded-[2rem] border border-border bg-panel p-6 shadow-telemetry">
          ...
          <h2 className="mt-2 font-display text-3xl">Decision tiles</h2>
          <p className="mt-3 max-w-2xl text-sm leading-6 text-muted">
            Sort the menu by urgency, profit contribution, or cost pressure, then open the dish that needs the next decision.
          </p>
        </div>

        <section className="rounded-[2rem] border border-border bg-panel p-6 shadow-telemetry">
          <p className="text-xs uppercase tracking-[0.2em] text-muted">Menu health snapshot</p>
          <h3 className="mt-2 font-display text-3xl">Current split</h3>
          <div className="mt-6">
            <MenuHealthBar
      ...
      <section className="rounded-[2rem] border border-border bg-panel p-6 shadow-telemetry">
        <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-muted">Control strip</p>
            <h2 className="mt-2 font-display text-3xl">Filter and sort the menu</h2>
          </div>
